...
from ..bin_paths import ytdlp_path, ytdlp_extra_args, _NOWWIN
from ..config import get_use_transcription
from .local_file import is_local_file
from .transcriber import TranscriptionError, cloud_transcribe_to_srt, resolve_transcription_engine


def fetch_video_metadata(url, cookie_browser=None, logger=None, process_tracker=None):
    """Fetch uploader/channel name and video title from a media URL.
    ...
    Returns:
        (uploader_name, video_title) tuple of strings
    ...
    if result.returncode != 0:
        ...
        if "Sign in to confirm" in stderr or "LOGIN_REQUIRED" in stderr:
            ...
            if not cookie_browser:
                cookie_hint = " Try selecting a browser cookie (e.g. Firefox) in the Process tab."
            raise RuntimeError(
        ...
        if "HTTP Error 413" in stderr or "Failed to extract any player response" in stderr:
            cookie_hint = ""
            if not cookie_browser:
                cookie_hint = " Also try selecting a browser cookie (e.g. Firefox) in the Process tab."
            raise RuntimeError(
                "yt-dlp may be outdated (YouTube returned HTTP 413). "
                f"Go to Settings \u2192 Check for Updates to update yt-dlp.{cookie_hint}"
            )
...
def download_youtube(url, output_dir, progress_callback=None, cookie_browser=None, sub_lang="ja",
    ...
    if not srt_path.exists():
        ...
        if force_transcription or get_use_transcription():
            engine = resolve_transcription_engine(sub_lang)
            if logger:
            ...
            if engine == "reazonspeech":
                ...
                if progress_callback:
                    label = (f"Force transcribing audio with {engine_label}..." if force_transcription
                             else f"No subtitles found, transcribing audio with {engine_label}...")
                    progress_callback(label)
            ...
            raise RuntimeError(
                ...
                "Enable 'Use Transcription Fallback' or 'Force Transcription' in the Process tab."
            )
